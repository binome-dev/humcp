...
class TestCreateApp:
    ...
    def test_discovers_and_registers_tools(self, sample_tools, empty_config):
        """Should discover and register tools."""
        app = create_app(tools_path=str(sample_tools), config_path=str(empty_config))
        client = TestClient(app)
        resp = client.get("/tools")
        assert resp.json()["total_tools"] == 3

    def test_tool_execution(self, sample_tools, empty_config):
    ...
    def test_category_endpoint(self, sample_tools, empty_config):
