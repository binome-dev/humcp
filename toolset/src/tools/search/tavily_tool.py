...
async def tavily_web_search(
    ...
    try:
        ...
        if not resolved_api_key:
            return TavilyWebSearchResponse(
                success=False,
                error="Search API not configured. Contact administrator.",
            )
        ...
        if not results:
            return TavilyWebSearchResponse(success=False, error="No results found.")
        ...
        return TavilyWebSearchResponse(success=True, data=data)
    except Exception as e:
        logger.exception("Tavily search failed")
        return TavilyWebSearchResponse(
            success=False, error=f"Tavily search failed: {str(e)}"
        )
...
async def tavily_extract(
    ...
    Extracts clean text content from web pages, useful for reading articles,
    ...
    Returns:
        Extracted text content for each URL.
    """
    try:
        resolved_api_key = os.getenv("TAVILY_API_KEY")
        if not resolved_api_key:
        ...
        if not urls:
            return TavilyExtractResponse(
                success=False, error="Please provide at least one URL to extract."
            )
        ...
        logger.info("Tavily extract urls=%d", len(urls))
        ...
        data = TavilyExtractData(results=results, failed_results=failed)
        logger.info("Tavily extract complete results=%d", len(results))
        return TavilyExtractResponse(success=True, data=data)
    except Exception as e:
        logger.exception("Tavily extract failed")
        return TavilyExtractResponse(
            success=False, error=f"Tavily extract failed: {str(e)}"
        )
