...
def _get_ses_client(region: str | None = None):
    ...
    return boto3.client("ses", region_name=resolved_region)
...
async def aws_ses_send_email(
    ...
    try:
        ...
        if err:
            return SesSendEmailResponse(success=False, error=err)

        resolved_from = from_addr or os.getenv("AWS_SES_FROM_EMAIL")
...
async def aws_ses_list_identities(
    ...
    try:
        err = _check_aws_creds()
        if err:
            return SesListIdentitiesResponse(success=False, error=err)
...
async def aws_ses_get_send_statistics(
    ...
    try:
        err = _check_aws_creds()
        if err:
            return SesSendStatisticsResponse(success=False, error=err)
...
async def aws_ses_verify_email(
    ...
    try:
        err = _check_aws_creds()
        if err:
            return SesVerifyEmailResponse(success=False, error=err)
