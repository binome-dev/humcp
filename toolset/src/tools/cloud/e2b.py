...
def _check_configured() -> str | None:
    """Return an error message if E2B is not configured, else None."""
    if not os.getenv("E2B_API_KEY"):
        return "E2B API key not configured. Set E2B_API_KEY."
    return None
...
async def e2b_run_code(
    ...
    try:
        ...
        if err:
            return E2BRunCodeResponse(success=False, error=err)
...
async def e2b_run_command(
    ...
    try:
        err = _check_configured()
        if err:
            return E2BRunCommandResponse(success=False, error=err)
...
async def e2b_upload_file(
    ...
    try:
        err = _check_configured()
        if err:
            return E2BUploadFileResponse(success=False, error=err)
...
async def e2b_download_file(
    ...
    try:
        err = _check_configured()
        if err:
            return E2BDownloadFileResponse(success=False, error=err)
...
async def e2b_list_files(
    ...
    try:
        err = _check_configured()
        if err:
            return E2BListFilesResponse(success=False, error=err)
