...
def _get_zendesk_config() -> tuple[str | None, tuple[str, str] | None, str | None]:
    """Build Zendesk API configuration from environment variables.

    Returns:
        A tuple of (base_url, auth_tuple, error_message).
    """
    ...
    return base_url, auth, None


def _parse_ticket(ticket: dict) -> ZendeskTicketData:
...
async def zendesk_create_ticket(
    ...
    """
    try:
        base_url, auth, error = _get_zendesk_config()
...
async def zendesk_search_tickets(
    ...
    try:
        ...
        tickets = [_parse_ticket(ticket) for ticket in result.get("results", [])]
