...
def _get_calcom_config() -> tuple[str | None, str]:
    """Resolve Cal.com API key and base URL from environment."""
...
def _check_configured() -> str | None:
    """Return an error message if Cal.com is not configured, else None."""
    if not os.getenv("CALCOM_API_KEY"):
        return "Cal.com API not configured. Set CALCOM_API_KEY."
    return None
...
async def calcom_list_bookings(
    ...
    try:
        ...
        if err:
            return CalcomListBookingsResponse(success=False, error=err)

        api_key, base_url = _get_calcom_config()
...
async def calcom_create_booking(
    ...
    try:
        err = _check_configured()
        if err:
            return CalcomCreateBookingResponse(success=False, error=err)
...
async def calcom_cancel_booking(
    ...
    try:
        err = _check_configured()
        if err:
            return CalcomCancelBookingResponse(success=False, error=err)
...
async def calcom_reschedule_booking(
    ...
    try:
        err = _check_configured()
        if err:
            return CalcomRescheduleBookingResponse(success=False, error=err)
...
async def calcom_get_availability(
    ...
    try:
        err = _check_configured()
        if err:
            return CalcomAvailabilityResponse(success=False, error=err)
...
async def calcom_list_event_types() -> CalcomListEventTypesResponse:
    ...
    try:
        err = _check_configured()
        if err:
            return CalcomListEventTypesResponse(success=False, error=err)
