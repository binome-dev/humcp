...
from src.tools.data.schemas import (
    ...
    DataFrameDetailedInfo,
    DataFrameInfo,
    DataFrameOperationData,
    DataFrameOperationResponse,
    DeleteDataFrameData,
    DeleteDataFrameResponse,
    ExportDataFrameData,
    ExportDataFrameResponse,
    GetDataFrameInfoResponse,
...
async def create_pandas_dataframe(
    ...
    try:
        ...
        if manager.dataframe_exists(dataframe_name):
            return CreateDataFrameResponse(
                success=False,
        ...
        return CreateDataFrameResponse(success=True, data=data)

    except Exception as e:
        return CreateDataFrameResponse(
            success=False, error=f"Error creating DataFrame: {str(e)}"
        )
...
async def run_dataframe_operation(
    ...
    try:
        ...
        if dataframe is None:
            return DataFrameOperationResponse(
                success=False,
        ...
        return DataFrameOperationResponse(
            success=False, error=f"Error running operation: {str(e)}"
        )


@tool()
async def list_dataframes() -> ListDataFramesResponse:
    """
    ...
    try:
        ...
        return ListDataFramesResponse(success=True, data=dataframes_info)

    except Exception as e:
        return ListDataFramesResponse(success=False, error=str(e))


@tool()
async def get_dataframe_info(dataframe_name: str) -> GetDataFrameInfoResponse:
    """
    ...
    try:
        ...
        if dataframe is None:
            return GetDataFrameInfoResponse(
                success=False,
                error=f"DataFrame '{dataframe_name}' not found",
            )

        data = DataFrameDetailedInfo(
            name=dataframe_name,
            shape=dataframe.shape,
            columns=list(dataframe.columns),
            dtypes={col: str(dtype) for col, dtype in dataframe.dtypes.items()},
        ...
        return GetDataFrameInfoResponse(success=True, data=data)

    except Exception as e:
        return GetDataFrameInfoResponse(success=False, error=str(e))


@tool()
async def delete_dataframe(dataframe_name: str) -> DeleteDataFrameResponse:
    """
    ...
    try:
        ...
        if not manager.dataframe_exists(dataframe_name):
            return DeleteDataFrameResponse(
                success=False,
                error=f"DataFrame '{dataframe_name}' not found",
            )
        ...
        return DeleteDataFrameResponse(
            success=True,
            data=DeleteDataFrameData(
                message=f"DataFrame '{dataframe_name}' deleted successfully"
            ),
        )

    except Exception as e:
        return DeleteDataFrameResponse(success=False, error=str(e))
...
async def export_dataframe(
    ...
    try:
        ...
        if dataframe is None:
            return ExportDataFrameResponse(
                success=False,
                error=f"DataFrame '{dataframe_name}' not found",
            )
        ...
        return ExportDataFrameResponse(
            success=False, error=f"Error exporting DataFrame: {str(e)}"
        )
