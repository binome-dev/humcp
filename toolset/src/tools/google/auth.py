...
def get_credentials(scopes: list[str]) -> Credentials | None:
    ...
    if TOKEN_PATH.exists():
        try:
            ...
            logger.warning("Cannot access token file: %s", e)
            creds = None
        except (json.JSONDecodeError, ValueError, KeyError) as e:
            logger.warning("Invalid token file format: %s", e)
            creds = None
        except Exception as e:
            logger.error("Unexpected error loading token: %s", e, exc_info=True)
            creds = None
    ...
    if creds and creds.expired and creds.refresh_token:
        try:
            ...
            logger.warning("Token refresh failed (re-auth required): %s", e)
            creds = None
        except Exception as e:
...
def _run_auth_flow(scopes: list[str]) -> Credentials | None:
    ...
    try:
        # Only allow http for localhost in non-production environments
        # This is required for local OAuth flow but should not be enabled in production
        ...
        logger.error("OAuth flow failed (network/server error): %s", e)
        return None
    except Exception as e:
        logger.error("OAuth flow failed: %s", e, exc_info=True)
        return None
...
async def run_google_api(
    ...
    """Run a synchronous Google API call with standard error handling.

    Args:
        operation_name: Name of the operation for logging (e.g., "gmail_search").
        sync_func: Synchronous function to execute in thread pool.
        tool_logger: Logger to use for this operation.

    Returns:
        Dict with {"success": True, "data": result} or {"success": False, "error": str}.
    """
    import asyncio

    try:
        tool_logger.info(operation_name)
        result = await asyncio.to_thread(sync_func)
        return {"success": True, "data": result}
    except Exception as e:
        tool_logger.exception(f"{operation_name} failed")
