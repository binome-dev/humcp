...
@tool()
async def google_docs_search(query: str, max_results: int = 25) -> DocsSearchResponse:
    ...
    try:
        ...
        documents = await asyncio.to_thread(_search)
        return DocsSearchResponse(
            success=True,
            data=DocsSearchData(documents=documents, total=len(documents)),
        )
    except Exception as e:
        logger.exception("docs_search failed")
        return DocsSearchResponse(success=False, error=str(e))


@tool()
async def google_docs_get_content(document_id: str) -> DocsGetContentResponse:
    ...
    try:
        ...
        return DocsGetContentResponse(success=True, data=result)
    except Exception as e:
        logger.exception("docs_get_content failed")
        return DocsGetContentResponse(success=False, error=str(e))


@tool()
async def google_docs_create(title: str, content: str = "") -> DocsCreateResponse:
    ...
    try:
        ...
        return DocsCreateResponse(success=True, data=result)
    except Exception as e:
        logger.exception("docs_create failed")
        return DocsCreateResponse(success=False, error=str(e))


@tool()
async def google_docs_append_text(
    ...
    try:
        ...
        def _append():
            ...
            return DocsAppendTextData(updated=True, document_id=document_id)

        logger.info("docs_append_text doc_id=%s", document_id)
        result = await asyncio.to_thread(_append)
        return DocsAppendTextResponse(success=True, data=result)
    except Exception as e:
        logger.exception("docs_append_text failed")
        return DocsAppendTextResponse(success=False, error=str(e))
...
async def google_docs_find_replace(
    ...
    try:
        ...
        return DocsFindReplaceResponse(success=True, data=result)
    except Exception as e:
        logger.exception("docs_find_replace failed")
        return DocsFindReplaceResponse(success=False, error=str(e))


@tool()
async def google_docs_list_in_folder(
    folder_id: str, max_results: int = 50
) -> DocsListInFolderResponse:
    ...
    try:
        ...
        return DocsListInFolderResponse(
            success=True,
            data=DocsListInFolderData(documents=documents, total=len(documents)),
        )
    except Exception as e:
        logger.exception("docs_list_in_folder failed")
        return DocsListInFolderResponse(success=False, error=str(e))
