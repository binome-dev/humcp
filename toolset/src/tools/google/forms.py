...
@tool()
async def google_forms_list_forms(max_results: int = 25) -> FormsListResponse:
    ...
    try:
        ...
        return FormsListResponse(success=True, data=result)
    except Exception as e:
        logger.exception("forms_list_forms failed")
        return FormsListResponse(success=False, error=str(e))


@tool()
async def google_forms_get_form(form_id: str) -> FormsGetFormResponse:
    ...
    try:
        ...
        def _get():
            ...
            for item in form.get("items", []):
                ...
                questions.append(FormQuestion(**q_info))
        ...
        return FormsGetFormResponse(success=True, data=result)
    except Exception as e:
        logger.exception("forms_get_form failed")
        return FormsGetFormResponse(success=False, error=str(e))


@tool()
async def google_forms_create_form(
    title: str, document_title: str = ""
) -> FormsCreateFormResponse:
    ...
    try:
        ...
        return FormsCreateFormResponse(success=True, data=result)
    except Exception as e:
        logger.exception("forms_create_form failed")
        return FormsCreateFormResponse(success=False, error=str(e))


@tool()
async def google_forms_list_responses(
    form_id: str, max_results: int = 100
) -> FormsListResponsesResponse:
    ...
    try:
        ...
        result = await asyncio.to_thread(_list)
        return FormsListResponsesResponse(success=True, data=result)
    except Exception as e:
        logger.exception("forms_list_responses failed")
        return FormsListResponsesResponse(success=False, error=str(e))


@tool()
async def google_forms_get_response(
    ...
    try:
        ...
        def _get():
            ...
            for question_id, answer_data in response.get("answers", {}).items():
                ...
                if "textAnswers" in answer_data:
                    ...
                    answer_info["files"] = [
                        FormFileAnswer(id=f.get("fileId"), name=f.get("fileName"))
                        for f in answer_data["fileUploadAnswers"].get("answers", [])
                    ]

                answers.append(FormAnswer(**answer_info))

            return FormResponseDetailed(
        ...
        result = await asyncio.to_thread(_get)
        return FormsGetResponseResponse(success=True, data=result)
    except Exception as e:
        logger.exception("forms_get_response failed")
        return FormsGetResponseResponse(success=False, error=str(e))
