...
class RouteGenerator:
    ...
    def _register_tool_route(self, app: FastAPI, tool: Tool):
        ...
        tool_endpoint.__name__ = f"execute_{tool.name}"
        tool_endpoint.__doc__ = tool.description or f"Execute {tool.name} tool"
    ...
    def create_info_routes(self, app: FastAPI):
                ...
                "tools": [
                    {
                        ...
                        "full_name": tool["full_name"],
                        "description": tool["description"],
                        "parameters": self.tools[tool["full_name"]].inputSchema if hasattr(self.tools[tool["full_name"]], 'inputSchema') else {},
                        "endpoint": tool["endpoint"]
                    }
                    for tool in category_data["tools"]
                ]
        ...
        async def get_tool_info(category: str, tool_name: str):
            ...
            if full_tool_name not in self.tools:
                raise HTTPException(status_code=404, detail=f"Tool '{full_tool_name}' not found")
