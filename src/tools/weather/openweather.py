...
async def _resolve_api_key() -> str | None:
    """Return the OpenWeatherMap API key via credential resolution."""
    return await resolve_credential("OPENWEATHER_API_KEY")
    ...
    try:
        ...
        if not api_key:
            return CurrentWeatherResponse(
                success=False,
                error="OpenWeatherMap API not configured. Set OPENWEATHER_API_KEY environment variable.",
            )
