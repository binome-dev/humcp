...
async def _get_confluence_client() -> tuple[Confluence | None, str | None]:
    """Create a Confluence client from environment variables.

    Returns:
        A tuple of (client, error_message).
    """
    url = await resolve_credential("CONFLUENCE_URL")
    username = await resolve_credential("CONFLUENCE_USERNAME")
    api_token = await resolve_credential("CONFLUENCE_API_TOKEN")
...
async def confluence_get_page(page_id: str) -> ConfluencePageResponse:
    ...
    """
    try:
        client, error = await _get_confluence_client()
        ...
        base_url = await resolve_credential("CONFLUENCE_URL") or ""
