...
def _get_airflow_config(
    ...
    """Resolve Airflow base URL and authentication.
...
async def airflow_list_dags(
    ...
    """
    try:
        username = await resolve_credential("AIRFLOW_USERNAME")
        password = await resolve_credential("AIRFLOW_PASSWORD")
        base_url, auth = _get_airflow_config(username, password)
