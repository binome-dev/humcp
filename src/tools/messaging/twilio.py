...
def _get_client(account_sid: str, auth_token: str) -> Client:
    """Create a Twilio client from the provided credentials."""
...
async def twilio_send_sms(
    ...
    """
    try:
        account_sid = await resolve_credential("TWILIO_ACCOUNT_SID")
        auth_token = await resolve_credential("TWILIO_AUTH_TOKEN")
        if not account_sid or not auth_token:
            return SendSmsResponse(
                success=False,
                error="Twilio not configured. Set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN environment variables.",
            )

        client = _get_client(account_sid, auth_token)
        sender = from_number or await resolve_credential("TWILIO_FROM_NUMBER")
        if not sender:
            return SendSmsResponse(
                success=False,
