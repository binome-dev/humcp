...
def _get_spider_client(api_key: str) -> ExternalSpider:
    ...
    return ExternalSpider(api_key=api_key)
...
async def spider_scrape(url: str) -> ScrapeResponse:
    ...
    try:
        ...
        api_key = await resolve_credential("SPIDER_API_KEY")
        if not api_key:
            return ScrapeResponse(
                success=False,
                error="Spider API not configured. Set SPIDER_API_KEY.",
            )

        app = _get_spider_client(api_key)
