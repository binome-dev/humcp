...
def _get_scrapegraph_client(api_key: str) -> Client:
...
async def scrapegraph_scrape(
    ...
    try:
        ...
        api_key = await resolve_credential("SGAI_API_KEY")
        if not api_key:
            return StructuredScrapeResponse(
                success=False,
                error="ScrapeGraph API not configured. Set SGAI_API_KEY.",
            )

        client = _get_scrapegraph_client(api_key)
