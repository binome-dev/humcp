...
def _get_client(api_key: str) -> AsyncZep:
    """Create an async Zep client using the provided API key.

    Args:
        api_key: The Zep API key.

    Returns:
        A configured AsyncZep client instance.
    """
...
async def zep_add_memory(
    ...
    try:
        ...
        api_key = await resolve_credential("ZEP_API_KEY")
        if not api_key:
            return ZepAddMemoryResponse(
                success=False, error="ZEP_API_KEY not configured."
            )
