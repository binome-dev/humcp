...
def _get_client(
    project_id: str | None = None, resolved_project_id: str | None = None
) -> bigquery.Client:
    ...
    Args:
        project_id: Optional Google Cloud project ID override from function argument.
        resolved_project_id: Optional project ID resolved from credentials.

    Returns:
        An authenticated BigQuery client.
    """
    resolved_project = project_id or resolved_project_id
...
async def google_bigquery_query(
    ...
    try:
        ...
        gcp_project = await resolve_credential("GOOGLE_CLOUD_PROJECT")

        def _execute() -> dict[str, Any]:
            client = _get_client(project_id, gcp_project)
